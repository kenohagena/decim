...
choice_k ~ LN_k-1 + LLR_k-1*CPP_k-1 + LLR_k-1*-|psi|_k-1 + LLR_k

Sanity Check model:

LN_k ~ LN_k-1 + LLR_k-1*CPP_k-1 + LLR_k-1*-|psi|_k-1 + LLR_k
'''
...
class Choiceframe(object):
    ...
    def __init__(self, subject, session, run, task,
        ...
        self.n_samples = n
        self.parameters = ['LLR', 'surprise', 'psi', 'belief']
...
def execute(subject, session, run, task,
    ...
    c.kernel_samples(parameter='belief')
    c.merge()
