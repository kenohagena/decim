...
for sub in range(1, 23):
    for ses in range(1, 4):
        try:
            s = pd.read_csv(expanduser('/Users/kenohagena/Flexrule/fmri/analyses/bids_stan_fits_300518/sub-{0}_ses-{1}_stanfit.csv'.format(sub, ses)))
            dr = {'vmode': statmisc.mode(s.V.values, 50), 'vupper': statmisc.hdi(s.V.values)[1],
                  'vlower': statmisc.hdi(s.V.values)[0], 'hmode': statmisc.mode(s.H.values, 50),
...
summary.to_csv('/Users/kenohagena/Flexrule/fmri/analyses/bids_stan_fits_300518/summary_stan_fits.csv')
...
f.savefig('HV_flexrule_stan_fits.png', dpi=160)
'''
