...
for subject in {1..22}; do
    ...
    if [ ! -d "$subdir" ]; then
        ...
        for maskfile in $atlas_dir/selected/*.nii.gz; do
            ...
            warpfile=/Volumes/flxrl/fmri/completed_preprocessed/sub-"$subject"/fmriprep/sub-"$subject"/anat/sub-"$subject"_T1w_space-MNI152NLin2009cAsym_target-T1w_warp.h5
            reference_file=/Volumes/flxrl/fmri/completed_preprocessed/sub-"$subject"/fmriprep/sub-"$subject"/anat/sub-"$subject"_T1w_preproc.nii.gz
