...
class Behav_Dataframes(object):
    ...
    def inference_run(self, summary):

        logs = gc.load_logs_bids(self.sub, self.ses, self.bids_path)
        logs = logs[1]
        for df in logs:
            run = str(df.iloc[25].block)[0]  # '25' is quite arbitrary
            belief = gl.belief(df, H=summary.loc[(summary.subject == self.subject) & (summary.session == self.session)].hmode.values[0], ident='event')
            df['belief'] = belief
            ...
            df = df.sort_index(by='onset').to_hdf(join(self.out_path, 'behav_{0}_{1}.hdf'.format(self.subject, self.session)), key=run)
    ...
    def instructed(self):
        ...
        df = df.loc[df.event.isin(['IR_STIM', 'IR_REWARDED_RULE', 'IR_TRIAL_FIXON',
                                   'CHOICE_TRIAL_ONSET',
                                   'CHOICE_TRIAL_STIMOFF',
                                   'CHOICE_TRIAL_RESP'])]
        ...
        df.to_hdf(join(self.out_path, 'behav_{0}_{1}.hdf'.format(self.subject, self.session)), key=run)
