...
def fix_keys():
    ...
    for H in [.01, .05, .1, .15, .2, .25, .3, .35, .4, .45]:
        for model, parameters in zip(['gvfix', 'vfix'], [['H', 'V'], ['H', 'gen_var']]):
            for i in range(20):
                yield(H, 1, 1, i, 'H', models[model], model, parameters)

    for gen_var in [1, 1.5, 2, 2.5, 3, 3.5]:
        for i in range(20):
            yield(.015, 1, gen_var, i, 'gen_var', models['vfix'], 'vfix', ['H', 'gen_var'])

def execute(H, V, gv, i, var, model, fixed_variable, parameters):
