...
class PupilFrame(object):
    ...
    def __init__(self, sub, ses, runs, flex_dir, pupil_frame=None):
        self.sub = sub
        self.ses = ses
        ...
        inf_runs = ['inference_run-4',
                    ...
                    'instructed_run-7',
                    'instructed_run-8']
        self.run_index = runs - 4
        self.run = inf_runs[self.run_index]
    ...
    def get_messages(self, m):
        ...
        for message_label in mess:
            ...
            #columns[message_label]['trial_id'] = [int(i[9:]) for i in message_frame.loc[~message_frame[message_label].isnull(), 'trialid '].values]

        return pd.concat(list(columns.values()))
    ...
    def finish(self, out_dir=None):
        ...
        self.z_score()
        if out_dir is None:
            out_dir = join('/Volumes/flxrl/FLEXRULE/', 'pupil', 'linear_pupilframes_manual')
        slu.mkdir_p(out_dir)
        self.pupil_frame.to_hdf(join(out_dir, 'pupilframe_{}_{}_{}.hdf'.format(self.subject, self.session, self.run)), key='df', mode='w')
