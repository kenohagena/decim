...
class Choiceframe(object):
    ...
    def choice_pupil(self, artifact_threshhold=.2, choicelock=True, tw=4500):
        ...
        for run in self.runs:
            pupil_frame = pd.read_hdf(join(self.flex_dir, 'pupil', 'linear_pupilframes_manual',
                                           'pupilframe_{0}_{1}_{2}.hdf'.format(self.subject, self.session, run)))
            pupil_frame['run'] = run
            frames.append(pupil_frame)
        pupil_frame = pd.concat(frames, ignore_index=True)
        df = pupil_frame.loc[:, ['message', 'biz', 'message_value', 'blink', 'run', 'trial_id']]
        ...
        for choice_trial in df.loc[df.message == "CHOICE_TRIAL_ONSET"].index:
            ...
            if len(df.iloc[onset: onset + 3500, :].loc[df.message == 'RT', 'message_value']) == 0:
                ...
                choice_interp = df.loc[np.arange(onset + resp - 1000, onset + resp + 1500).astype(int), 'blink'].values
                gratingpupil.append(gratinglock)
                choicepupil.append(choicelock)
                parameters.append(choice_parameters)
                choice_interpol.append(choice_interp)
    ...
    def merge(self):
        ...
        interpol.columns = pd.MultiIndex.from_product([['pupil'], ['choice_interpol'], range(interpol.shape[1])], names=['source', 'type', 'name'])
        singles = [grating, choice, choices, points, paras, interpol]
    ...
    def defit_clean(self):
        clean = self.master
        clean = clean.loc[df.pupil.parameters.blink == 0]
        ...
        err = []
        ts = []
        for tmax in tqdm(np.arange(-0.3, 0, 0.01)):
        ...
        clean.to_hdf('/Volumes/flxrl/FLEXRULE/pupil/choice_epochs/clean_epochs_{0}_ses-{1}.hdf'.format(self.subject, self.ses), mode='w', key='clean')
