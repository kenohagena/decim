...
'''
Execute and parallelize simulation of data and recovery of parameter values using Stan on SLURM.

1. "fit_keys": loops through parameter combinations for data simulation
2. "execute" takes such combinations of parameters as input and
    a) compiles the stan model
    b) simulates data using "pointsimulation.py"
    c) fits the model tp the data using pystan
    d) outputs a dictionary with the parameter estimates
'''
...
models = {'vfix': 'stan_models/inv_glaze_b_fixV.stan',                          # 'vfix': internal noise V is fixed to 1, generative variance allowed to vary
          'gvfix': 'stan_models/inv_glaze_b_fixgen_var.stan'}                   # 'gvfix' the other way around
...
def execute(H, V, gv, i, var, model, fixed_variable, parameters, isi, trials):
    ...
    compilefile = join('/work/faty014',
                       model.replace('/', '') + 'stan_compiled.pkl')
    try:
