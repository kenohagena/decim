...
def surface_data(input_dir, lateral_params):
    ...
    mag_lat.to_hdf(join(input_dir, 'surface_textures_average_t.hdf'), key='mag_lat')
...
def submit():
    for sub in range(1, 23):
        slu.pmap(par_execute, [keys(sub)], walltime='2:55:00',
                 memory=60, nodes=1, tasks=2, name='linreg_voxel')


#surface_data('/Volumes/flxrl/FLEXRULE/fmri/linreg_voxel/surface_textures', ['belief', 'rresp', 'response', 'LLR'])
print(keys(3))
