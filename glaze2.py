...
def path_name(sub_code, session, phase, block):
    ...
    directory = os.path.join('/Users/kenohagena/Documents/Data/', "{}".format(sub_code), "{}".format(session), 'PH_' + "{}".format(phase) + 'PH_' + "{}".format(block))
...
def former_path_name(sub_code, session, phase, block):
    ...
    if block == 1:
        ...
        directory = os.path.join('/Users/kenohagena/Documents/Data/', sub_code, session, 'PH_' + "{}".format(phase) + 'PH_' + "{}".format(block - 1))
    for file in os.listdir(directory):
        if file.endswith(".mat"):
            return os.path.join(directory, file)
...
def load_np_unfiltered(sub_code, session, phase, block, key="p"):
    ...
    def foo(item):
        return {'time': item[0, 0][0, 0], 'message': item[0, 1][0], 'value': item[0, 2].ravel()[0],
                'block': item[0, 3][0, 0],
                'phase': item[0, 4][0, 0]}

    pl = [foo(log[i, 0]) for i in range(log.shape[0])]
...
# function to decide automatically which of the two options above shall be applied
...
def p_loc(DataFrame):
    ...
    return np.array(df.query('message=="GL_TRIAL_LOCATION"').value.astype(float))
...
def p_loc_ind(DataFrame):
    ...
    df = DataFrame
    df.message = df.message.astype(str)
    index1 = np.array(df.query('message=="GL_TRIAL_LOCATION"').index.astype(float))
...
def answers(DataFrame):
    ...
    0 for x and 1 for m.'''
    df = DataFrame
    choice = np.array(df.query('message=="CHOICE_TRIAL_RESP"').value.astype(float))
    return choice
...
def answer_ind(DataFrame):
    ...
    df = DataFrame
    choice_ind = np.array(df.query('message=="CHOICE_TRIAL_RESP"').index.astype(float))
    return choice_ind
...
def rule_resp(DataFrame):
    ...
    df = DataFrame
    rresp = np.array(df.query('message=="CHOICE_TRIAL_RULE_RESP"').value.astype(float))
    return rresp
...
def stim(DataFrame):
    ...
    df = DataFrame
    stimulus = np.array(df.query('message=="GL_TRIAL_STIM_ID"').value.astype(float))
    return stimulus
...
def dist_source(DataFrame):
    ...
    dist_ind = np.array(df.query('message=="GL_TRIAL_GENSIDE"').index.astype(float))
    # mask to only contain 'gen side' in decision trial, thus only if 'stim_id' follows
    mask = []
...
def reward(DataFrame):
    ...
    df = DataFrame
    rewards = np.array(df.query('message=="GL_TRIAL_REWARD"').value.astype(float))
    return rewards
...
def mod_choice(DataFrame, H):
    ...
    returns choices model would have made before decision trial. Hazard rate optional argument.'''
...
def LLR(value, e1=0.5, e2=-0.5, sigma=1):
    ...
    LLR = np.log(norm.pdf(value, e1, sigma)) - np.log(norm.pdf(value, e2, sigma))
    return LLR
...
def prior(b_prior, H):
    ...
    psi = b_prior + np.log((1 - H) / H + np.exp(-b_prior)) - np.log((1 - H) / H + np.exp(b_prior))
    return psi
...
# function to filter out beliefs at relevant postitions, i.e. before decision trials
def filter_dec(x,DataFrame):
    ...
    # mask contains indices of p_loc in ALL TRIALS of p_locs before decision trial (thus only if 'stim_id' follows 4 logs later)
...
def map_rresp(x):
    ...
    rule response of subject is mapped as follows: 1 == vertical-left, 0 == vertical righ.'''
    return -(x>0).astype(float) + 1

def mod_suc(DataFrame, H):
    '''counts choices in which the model micmicks succesfully the subjects answers.
...
def ce_error(DataFrame, H):
    '''returns the cross=entropy error for a given input DataFrame dependent on the hazard rate H.'''
...
def optimal_H(DataFrame):
    ...
    def foo(x):
        return ce_error(DataFrame, x)
    o = opt.minimize_scalar(foo, bounds=(0, 1), method='bounded')
    return o
...
__version__ = '2.1.2'
#2.1.0
### Version sperated loading functions from calculating functions and extractung functions.
